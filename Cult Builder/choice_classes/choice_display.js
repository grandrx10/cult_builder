class Choice_Display{
    ...
    set_choices(choices_hashmap, cult_stats){
        ...
        for (var i in choices_hashmap){
            var dialogue_obj = new Dialogue(choices_hashmap[i][0], choices_hashmap[i][1])
            ...
            if (dialogue_obj.check_requirements(cult_stats)){
                ...
                x = windowWidth/2
                y = windowHeight/2 + count*90 - 2*num_choices*90/3
    ...
    check_interaction(mouse_x, mouse_y, text_box, character_manager, cult_stats){
        for (var i = 0; i < this.choices.length; i ++){
            if (this.choices[i].check_clicked(mouse_x, mouse_y)){
                character_manager.set_character_focus_dialogue_path(this.choices[i].get_if_triggered_text(), 0, text_box, character_manager, cult_stats)
